...
```{r, echo=T}
stratified_bootstrap <- function(DF, nboot = 500, estimator = "g.formula"){
  ...
  for (i in 1:nboot){
    ...
    if (estimator == "g.formula"){
      ...
      estimands <- c(estimands, compute_ipsw(RCT_RWD))
    } else if (estimator == "aipsw"){
      estimands <- c(estimands, compute_aipsw(RCT_RWD))
    } else { break }
...
ate <- compute_gformula(star[, c("gender", "race", "S", "Y", "A")])
bootstrap <- stratified_bootstrap(DF=star[, c("gender", "race", "S", "Y", "A")], estimator = "g.formula")
bootstrap_sorted <- sort(bootstrap)
CI_inf <- quantile(bootstrap_sorted, 0.025)
CI_sup <- quantile(bootstrap_sorted, 0.975)
...
```{r}
compute_ipsw <- function(DF){
  
  N <- nrow(DF)
  n <- nrow(DF[DF$S ==1, ])
  m <- nrow(DF[DF$S ==0, ])
  e1.hat <- mean(DF[DF$S ==1, "A"])
  ...
  rct <- temp[temp$S == 1,]
  scores <- (n/m)*rct$Y*rct$odds*( (rct$A/e1.hat) - ((1-rct$A)/(1-e1.hat)) ) 
  tau_ipsw <- mean(scores)
  se <- sd(scores)/sqrt(n)
  
  return(c("estimate" = tau_ipsw, "se" = se))
}
compute_ipsw(star[, c("gender", "race", "g1freelunch", "gkfreelunch", "g1surban", "g1freelunch", "gkfreelunch", "g1surban", "S", "Y", "A")])

```

```{r}
compute_aipsw <- function(DF){
  ...
  # p.fit  <- glm(S ~., family = binomial("logit"), data = temp[, !names(temp) %in% c("A", "Y")])
  # p <- predict(p.fit, type = "response", newdata = temp)
  propensity.model <- probability_forest(temp[, !names(temp) %in% c("A", "Y", "S")],
                                         as.factor(temp$S),
                                         num.trees = 500,
                                         min.node.size = 5)
  p <- predict(propensity.model, data = X_t)$predictions[,2]
  
  mu_1 <- lm(Y ~., data = temp[temp$S == 1 & temp$A == 1, !names(temp) %in% c("S", "A")])
  mu_0 <- lm(Y ~., data = temp[temp$S == 1 & temp$A == 0, !names(temp) %in% c("S", "A")])
    
  temp$mu_1_predict <- predict.lm(mu_1, newdata = temp[, !names(temp) %in% c("S", "A")])
  temp$mu_0_predict <- predict.lm(mu_0, newdata = temp[, !names(temp) %in% c("S", "A")])

  # Store odds
  temp$odds <- (1-p)/p
  
  # Keep only RCT for the rest of the calculus
  scores <- (n/m)*temp$odds*temp$S*( ((temp$Y - temp$mu_1_predict)*temp$A/e1.hat) - ((1-(temp$Y - temp$mu_1_predict)*temp$A)/(1-e1.hat)) ) + (1-temp$S)*(temp$mu_1_predict-temp$mu_0_predict)
  tau_aipsw <- mean(scores)
  se <- sd(scores)/sqrt(n)
  
  return(tau_aipsw)
  #return(c("estimate" = tau_aipsw, "se" = se))
}

ate <- compute_aipsw(star[, c("gender", "race", "g1freelunch", "gkfreelunch", "g1surban", "S", "Y", "A")])
bootstrap <- stratified_bootstrap(DF=star[, c("gender", "race", "g1freelunch", "gkfreelunch", "g1surban", "S", "Y", "A")], estimator = "aipsw")
bootstrap_sorted <- sort(bootstrap)
CI_inf <- quantile(bootstrap_sorted, 0.025)
CI_sup <- quantile(bootstrap_sorted, 0.975)
GENERALIZED_ATE_all_cov = c(Difference = ate, CI_inf = CI_inf, CI_sup = CI_sup)
...
ggplot(results, aes(x=names_recoded, y=Difference)) + 
  ...
  ggsave("./figures/bilan_star.pdf", width = 4, height = 2.5)
```
