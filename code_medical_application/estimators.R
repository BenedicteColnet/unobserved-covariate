...
compute_gformula_with_ranger <- function(data){
  
  mu_1 =  ranger(Y ~ ., 
                 num.trees = 1000,
                 data = data[data$S == 1 & data$A == 1, !names(data) %in% c("S", "A")],
                 splitrule = "variance")
  mu_0 =  ranger(Y ~ ., 
                 num.trees = 1000,
                 data = data[data$S == 1 & data$A == 0, !names(data) %in% c("S", "A")],
                 splitrule = "variance")
  
  
  mu_1_hat <- predict(mu_1, data = data[data$S==0, !names(data) %in% c("S", "A", "Y")])$predictions
  mu_0_hat <- predict(mu_0, data = data[data$S==0, !names(data) %in% c("S", "A", "Y")])$predictions
