...
shifts <- seq(-0.80, 0.60, by = 0.005)
deltas <- seq(-0.15, 0.25,by = 0.005) 
...
```{r}
library(ggrepel)
ggplot(data = hat_deltas_with_CI[hat_deltas_with_CI$covariate != "gender",],
             aes(x = shift,
                 y = delta,
                 label = covariate)) + 
  geom_contour_filled(data = heatmap_data, aes(x = shift, y = delta, z = -bias)) +
  theme_bw() +
  geom_pointrange(data = hat_deltas_with_CI[hat_deltas_with_CI$covariate != "gender",], aes(xmax = CI_sup_shift, xmin = CI_inf_shift), size = 0.3) +
  geom_pointrange(data = hat_deltas_with_CI[hat_deltas_with_CI$covariate != "gender",], aes(ymax = CI_sup_delta, ymin = CI_inf_delta), size = 0.3) +
  geom_label_repel(data = hat_deltas_with_CI[hat_deltas_with_CI$covariate != "gender",], aes(x = shift, y = delta, label = covariate), size = 5.5) +
...
```
This is how the shift can be interpreted:
